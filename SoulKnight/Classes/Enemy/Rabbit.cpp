...
Rabbit* Rabbit::create(GameScene* Scene, std::string Name)
{
	Rabbit* enemy = new Rabbit();
...
bool Rabbit::init(GameScene* Scene, std::string Name)
{
...
bool Rabbit::initData(GameScene* Scene, std::string Name)
{
	exploreScene = Scene;
	enemyName = Name;
	camp = AllCamp::ENEMY;
	...
	attackSpeed = RABBIT_ATTACKSPEED;
	hitPoints = RABBIT_HP;
	curHitPoints = hitPoints;    //数据具体化，不动用plist

	damageAbility = RABBIT_DAMAGE;
	moveSpeed = RABBIT_MOVESPEED;
	identityRadius = RABBIT_IDR;
	attackRadius = RABBIT_ATTACKR;

	alreadyDead = false;
	everAttack = false;


	setTexture("ArtDesigning/Sprite/Enemy/Rabbit/Rabbit1.png");
	loadAnimation();
...
bool Rabbit::loadAnimation()
{
	Animation* normalAnimation = Animation::create();
	for (int i = 1; i <= 2; i++)
	{
		auto frameName = String::createWithFormat("ArtDesigning/Sprite/Enemy/Rabbit/Rabbit%d.png", i);
...
bool Rabbit::attack()
{
	if (attackTarget)
	{
		...
		auto damage = Bullet::create("ArtDesigning/FlyingItem/Bullet/RabbitEffect.png",
			damageAbility,
			0,
			this,
			NULL);
		damage->setAttackMode(MELEE);
		damage->setScaleX(0.4);
		damage->setPosition(this->getPosition().x + 10 * cos(angle / 180 * M_PI),
			this->getPosition().y + 10 * sin(angle / 180 * M_PI));
